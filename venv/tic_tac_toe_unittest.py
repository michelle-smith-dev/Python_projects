...
class MyTestCase(unittest.TestCase):
    ...
    def test_update_board(self):
        ...
        # Retest the above for loop of 'O's' but do not fill in all 'O's and ensure assertion fails
        ttt = TicTacToe()
        m, n = 0, 0
        for ele_m in range(3):
            for ele_n in range(2):
                ttt.update_board(m, n, "O")
                # Add assert statement below after board is filled
                if ele_m == 2 and ele_n == 2:
                    break
            ...
            m += 1
            n = 0
        print("Failed board:", ttt.board)
        for ele_m in range(3):
            for ele_n in range(3):
                # with self.assertRaises(AssertionError):
                #     ttt.board[ele_m][ele_n] == 'O'
                assert ttt.board[ele_m][ele_n] == 'O'
